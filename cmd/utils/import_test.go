...
// TestImportChainReportsLocalInsertCondition covers the other half of the same contract: a file
// whose block this node's clock cannot place yet is a condition of this node, not a corrupt file,
// so the importer has to report the reason the classifier gives it instead of "invalid block N" -
// the same file may well import once the clock, rather than the file, has changed.
func TestImportChainReportsLocalInsertCondition(t *testing.T) {
	gspec := &core.Genesis{
		BaseFee: big.NewInt(params.InitialBaseFee),
		Config:  params.TestChainConfig,
	}
	...
	// A block dated far past the future queue's window, so insertChain reports
	// ErrLocalInsertCondition instead of parking it. ethash checks the timestamp before anything
	// else it verifies, so the rest of the header does not have to be mined.
	...
	file := filepath.Join(t.TempDir(), "future.rlp")
	f, err := os.Create(file)
	if err != nil {
		t.Fatalf("failed to create the import file: %v", err)
	}
	if err := rlp.Encode(f, block); err != nil {
		t.Fatalf("failed to write the block into the import file: %v", err)
	}
	f.Close()

	target, err := core.NewBlockChain(db, nil, gspec, ethash.NewFaker(), vm.Config{})
	if err != nil {
		t.Fatalf("failed to create the target chain: %v", err)
	}
	defer target.Stop()
	...
	err = ImportChain(target, file)
	if err == nil || !strings.HasPrefix(err.Error(), "cannot be imported: ") {
		t.Fatalf("unexpected error: have %v want the local condition and its reason", err)
	}
	if !strings.Contains(err.Error(), core.ErrLocalInsertCondition.Error()) {
		t.Fatalf("the local condition must keep its cause: have %v want %q", err, core.ErrLocalInsertCondition.Error())
	}
	if strings.Contains(err.Error(), "invalid block") {
		t.Fatalf("a condition of this node must not be reported as a bad block in the file: %v", err)
	}
}
