...
func TestInsertSideChainReportsFutureBlockAsLocal(t *testing.T) {
	...
	n, _, _, err := chain.insertSideChain(block, it, true)
	if !errors.Is(err, ErrLocalInsertAheadOfClock) {
		t.Fatalf("unexpected error: have %v want %v", err, ErrLocalInsertAheadOfClock)
	}
...
// TestInsertSideChainDoesNotCarryAnUnreadableBaseline covers the baseline the scan carries over
// from a canonical re-import. A record this node cannot read is dropped rather than kept: the
// value it would fall back to belongs to an earlier canonical block, so the segment would be
// weighed from below the fork point - understated, which is the direction that lets a heavier
// branch lose the comparison and never be rebuilt. A batch of canonical re-imports alone then
// accumulated nothing to compare with, which is the local condition reported after the scan.
func TestInsertSideChainDoesNotCarryAnUnreadableBaseline(t *testing.T) {
	chain, blocks := newInsertChainTester(t, nil, 6, 6) // head at #6

	// #4 keeps its record and is read first, so the scan holds a baseline. #5 is the last
	// canonical block the scan passes and the record it would carry over is the one that is gone.
	unreadable := blocks[4]
	rawdb.DeleteTd(chain.ChainDb(), unreadable.Hash(), unreadable.NumberU64())
	chain.hc.tdCache.Remove(unreadable.Hash())
	if td := chain.GetTd(unreadable.Hash(), unreadable.NumberU64()); td != nil {
		t.Fatalf("precondition: the record must be gone, have %v", td)
	}

	batch := blocks[3:5] // #4 and #5, both canonical and read back out of the database
	results := make(chan error, len(batch))
	for range batch {
		results <- consensus.ErrPrunedAncestor
	}
	...
	n, _, _, err := chain.insertSideChain(block, it, true)
	if !errors.Is(err, ErrLocalInsertCondition) {
		t.Fatalf("block %d: a segment this node cannot weigh at the fork point is a local condition: have %v want %v",
			n, err, ErrLocalInsertCondition)
	}
	if !IsLocalInsertError(err) {
		t.Fatalf("a record missing in this node says nothing about the peer: %v", err)
	}
	if want := 2; n != want {
		t.Fatalf("unexpected failing index: have %d want %d", n, want)
	}
