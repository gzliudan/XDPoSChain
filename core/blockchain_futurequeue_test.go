...
// invalid block is not parked, and the batch stops there instead of importing on.
...
// TestInsertChainStopsAtUnknownAncestorTail pins the tail contract: an unknown ancestor in
// the middle of a batch stops it, it stays non-local so the downloader may hold the peer to
// it, and the prefix that did import stays on the chain.
func TestInsertChainStopsAtUnknownAncestorTail(t *testing.T) {
