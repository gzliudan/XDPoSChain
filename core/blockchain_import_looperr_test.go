...
func TestInsertChainReportsMidBatchFailure(t *testing.T) {
	...
	// The block the loop stopped on is the one the peer is blamed for, so it belongs in the
	// bad-block database like the block that fails as the first of a batch. #3 is the block the
	// engine rejects here, at index 2 of the batch.
	if rawdb.ReadBadBlock(chain.ChainDb(), blocks[2].Hash()) == nil {
		t.Fatal("the block that stopped the batch was not recorded as a bad block")
	}
...
func TestInsertChainReportsMidBatchBodyFailure(t *testing.T) {
	...
	// The other trigger class is recorded the same way: #3 is the block whose body fails, at
	// index 2 of the batch, and it is a consensus failure rather than a local condition.
	if rawdb.ReadBadBlock(chain.ChainDb(), blocks[2].Hash()) == nil {
		t.Fatal("the block whose body validation stopped the batch was not recorded as a bad block")
	}
	for i := 2; i < len(blocks); i++ {
		if block := chain.GetBlockByNumber(blocks[i].NumberU64()); block != nil {
