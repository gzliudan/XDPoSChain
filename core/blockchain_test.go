...
// It also makes failVerifyEngine satisfy the proposedBlockHandler interface
// that procFutureBlocks asserts on the chain engine.
func (e *failVerifyEngine) HandleProposedBlock(chain consensus.ChainReader, header *types.Header) error {
	e.handleMu.Lock()
...
func (e *failVerifyEngine) lastHandled() *types.Header {
	e.handleMu.Lock()
	defer e.handleMu.Unlock()
...
func (e *failVerifyEngine) handleSnapshot() []*types.Header {
	e.handleMu.Lock()
	defer e.handleMu.Unlock()
...
func TestInsertChainQueuesWrappedFutureError(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			engine := &failVerifyEngine{Engine: ethash.NewFaker(), failErr: wrapped}
...
func TestInsertChainFutureBlocksBeyondEnqueueWindow(t *testing.T) {
	...
	// Simulate a 2s block period with the batch starting 20s ahead of the
	// clock: blocks 0..5 sit inside the 30s enqueue window, blocks 6.. jump
	// past it by a wide margin so the test does not race the wall clock
	// around the boundary.
	_, blocks, _ := GenerateChainWithGenesis(gspec, ethash.NewFaker(), 12, func(i int, gen *BlockGen) {
		if i < 6 {
			gen.header.Time = now + 20 + uint64(2*i)
		} else {
...
// TestInsertChainCapsFutureBatchEnqueue verifies that a single batch cannot
// park more than maxFutureBlocks blocks in the future queue: the first block
// beyond the cap fails the import — making the downloader drop the delivering
// peer — instead of silently filling the queue with unverified blocks.
func TestInsertChainCapsFutureBatchEnqueue(t *testing.T) {
	var (
		key, _  = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
		address = crypto.PubkeyToAddress(key.PublicKey)
		funds   = big.NewInt(1000000000000000)
	...
	// A batch longer than the future queue, with every timestamp kept inside
	// the enqueue window for the whole test run.
	_, blocks, _ := GenerateChainWithGenesis(gspec, ethash.NewFaker(), 300, func(i int, gen *BlockGen) {
	...
	n, err := chain.InsertChain(blocks)
	if n != maxFutureBlocks {
		t.Fatalf("unexpected failing index: have %d want %d", n, maxFutureBlocks)
	}
	if !errors.Is(err, errTooManyFutureBlocks) {
		t.Fatalf("unexpected error for block %d: %v", n, err)
	}
	if head := chain.CurrentBlock().Number.Uint64(); head != 0 {
		t.Fatalf("unexpected head number: have %d want 0", head)
	}
	if have := chain.futureBlocks.Len(); have != maxFutureBlocks {
		t.Fatalf("unexpected queue length: have %d want %d", have, maxFutureBlocks)
	}
	for i, block := range blocks {
		if queued, want := chain.futureBlocks.Contains(block.Hash()), i < maxFutureBlocks; queued != want {
			t.Fatalf("block %d queued = %v, want %v", block.NumberU64(), queued, want)
		}
		if bad := rawdb.ReadBadBlock(db, block.Hash()); bad != nil {
			t.Fatalf("block %d recorded as bad block", block.NumberU64())
...
// TestInsertChainFillsRemainingFutureQueueCapacity verifies that the batch
// future quota is the queue's remaining capacity, not a fresh maxFutureBlocks
// allowance: a delivery may fill the queue up but must never displace blocks
// parked by earlier deliveries (a full LRU would silently evict its oldest
// entries). Once the queue is full the import fails, blocks enqueued up to
// that point stay parked, and nothing is reported as a bad block.
func TestInsertChainFillsRemainingFutureQueueCapacity(t *testing.T) {
	var (
		key, _  = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
		address = crypto.PubkeyToAddress(key.PublicKey)
		funds   = big.NewInt(1000000000000000)
	...
	// A block parked by an earlier delivery: an orphan whose parent can never
	// resolve, so the background drain keeps it parked while it is still in
	// the future window.
	seed := types.NewBlockWithHeader(&types.Header{
	...
	// A fresh batch that on its own would just fit the empty queue.
	_, blocks, _ := GenerateChainWithGenesis(gspec, ethash.NewFaker(), maxFutureBlocks, func(i int, gen *BlockGen) {
		gen.header.Time = now + 10
	})
	...
	chain.futureBlocks.Add(seed.Hash(), seed)

	n, err := chain.InsertChain(blocks)
	if n != maxFutureBlocks-1 {
		t.Fatalf("unexpected failing index: have %d want %d", n, maxFutureBlocks-1)
	}
	if !errors.Is(err, errTooManyFutureBlocks) {
		t.Fatalf("unexpected error for block %d: %v", n, err)
	}
	...
	if !chain.futureBlocks.Contains(seed.Hash()) {
		t.Fatalf("earlier delivery's parked block %d displaced by the new batch", seed.NumberU64())
	}
	if bad := rawdb.ReadBadBlock(db, seed.Hash()); bad != nil {
		t.Fatalf("seeded block %d recorded as bad block", seed.NumberU64())
	}
}

// TestProcFutureBlocksKeepsStillFutureBlocksWhenQueueFull verifies that a
// still-future block parked in a FULL future queue survives the future-block
// retry: re-adding it does not grow the queue, so the capacity check must
// not reject it — a rejection would make procFutureBlocks treat it as a
// non-retryable failure and evict a perfectly valid block, leaving a silent
// height gap. Once the verification errors are lifted the queue drains and
// the head advances.
func TestProcFutureBlocksKeepsStillFutureBlocksWhenQueueFull(t *testing.T) {
	var (
		key, _  = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
		address = crypto.PubkeyToAddress(key.PublicKey)
		funds   = big.NewInt(1000000000000000)
	...
	if _, err := chain.InsertChain(blocks); !errors.Is(err, errTooManyFutureBlocks) {
		t.Fatalf("unexpected fill error: %v", err)
	}
	if have := chain.futureBlocks.Len(); have != maxFutureBlocks {
		t.Fatalf("queue not full after fill: have %d want %d", have, maxFutureBlocks)
	}

	// Retry the queue while every block is still future: nothing may be evicted.
	chain.procFutureBlocks()
	if have := chain.futureBlocks.Len(); have != maxFutureBlocks {
		t.Fatalf("future-block retry evicted a still-future block: have %d want %d", have, maxFutureBlocks)
	}
	for _, block := range blocks[:maxFutureBlocks] {
		if bad := rawdb.ReadBadBlock(db, block.Hash()); bad != nil {
			t.Fatalf("block %d recorded as bad block", block.NumberU64())
		}
	}

	// Give the background drain a couple of ticks: the queue must stay full.
	time.Sleep(250 * time.Millisecond)
	if have := chain.futureBlocks.Len(); have != maxFutureBlocks {
		t.Fatalf("background drain evicted a still-future block: have %d want %d", have, maxFutureBlocks)
	}

	// Lift the verification errors: the queue drains and the head advances.
	engine.failFrom.Store(0)
	deadline := time.Now().Add(10 * time.Second)
	for chain.CurrentBlock().Number.Uint64() < maxFutureBlocks {
		if time.Now().After(deadline) {
			t.Fatalf("queue did not drain: head %d, queue %d", chain.CurrentBlock().Number.Uint64(), chain.futureBlocks.Len())
		}
		time.Sleep(50 * time.Millisecond)
	}
	if have := chain.futureBlocks.Len(); have != 0 {
		t.Fatalf("queue not drained after import: have %d want 0", have)
	}
}
...
// TestInsertChainEvictsStaleFutureBlockOnRejectedEnqueue verifies that a block
// already sitting in the future queue is evicted when its re-enqueue fails the
// window check (possible after a wall-clock step back between the original
// park and this retry), so it cannot linger in the queue and be retried by
// procFutureBlocks forever.
func TestInsertChainEvictsStaleFutureBlockOnRejectedEnqueue(t *testing.T) {
	var (
		key, _  = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
		address = crypto.PubkeyToAddress(key.PublicKey)
		funds   = big.NewInt(1000000000000000)
	...
	// Timestamps far beyond the maxTimeFutureBlocks enqueue window.
	_, blocks, _ := GenerateChainWithGenesis(gspec, ethash.NewFaker(), 2, func(i int, gen *BlockGen) {
		gen.header.Time = now + 60 + uint64(i)
	})
	...
	// Seed the queue as if the block had been parked before the window moved.
	chain.futureBlocks.Add(blocks[0].Hash(), blocks[0])

	n, err := chain.InsertChain(blocks)
	if n != 0 {
		t.Fatalf("unexpected failing index: have %d want 0", n)
	}
	if err == nil || !strings.Contains(err.Error(), "future block timestamp") {
		t.Fatalf("unexpected error: %v", err)
	}
	if chain.futureBlocks.Contains(blocks[0].Hash()) {
		t.Fatalf("stale future block %d still queued", blocks[0].NumberU64())
	}
	if chain.futureBlocks.Contains(blocks[1].Hash()) {
		t.Fatalf("block %d queued after the failed enqueue", blocks[1].NumberU64())
	}
	if bad := rawdb.ReadBadBlock(db, blocks[0].Hash()); bad != nil {
		t.Fatalf("block %d recorded as bad block", blocks[0].NumberU64())
	}
}
...
func TestProcFutureBlocksHandlesHighestImportedCanonicalBlock(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			engine := &failVerifyEngine{Engine: ethash.NewFaker(), failNumber: tc.failNumber, failErr: tc.failErr}
...
func TestProcFutureBlocksEvictsUnretryableFutureBlocks(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "full-queue-poison-drains", poison: true, count: maxFutureBlocks + 1, expire: true},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Phase 1: deliver the batch while every block is future. A batch
			// longer than the queue capacity fails after filling the queue;
			// shorter batches report success with the whole batch parked.
			n, err := chain.InsertChain(blocks)
			if tc.count > maxFutureBlocks {
				if !errors.Is(err, errTooManyFutureBlocks) {
					t.Fatalf("unexpected error for batch beyond the queue capacity: %v", err)
				}
			} else if err != nil {
				t.Fatalf("failed to deliver batch: index %d err %v", n, err)
			}
			queued := tc.count
			if queued > maxFutureBlocks {
				queued = maxFutureBlocks
			}
			if have := chain.futureBlocks.Len(); have != queued {
				t.Fatalf("unexpected queue length after delivery: have %d want %d", have, queued)
			}
			...
			if !tc.expire {
				...
				if have := chain.futureBlocks.Len(); have != queued {
					t.Fatalf("future blocks evicted while still in the future: have %d want %d", have, queued)
				}
				for _, block := range blocks {
					if bad := rawdb.ReadBadBlock(db, block.Hash()); bad != nil {
			...
			if tc.count > maxFutureBlocks {
				// The highest parked block is reported exactly once; it stays in
				// the persisted bad-block list, which keeps only the highest
				// numbers, and the never-queued overflow block is not reported.
				if bad := rawdb.ReadBadBlock(db, blocks[queued-1].Hash()); bad == nil {
					t.Fatalf("garbage block %d not reported as bad", blocks[queued-1].NumberU64())
				}
				if bad := rawdb.ReadBadBlock(db, blocks[queued].Hash()); bad != nil {
					t.Fatalf("undelivered block %d reported as bad", blocks[queued].NumberU64())
				}
				return
			}
			// The small chain fits the persisted list in full: every parked
			// block was reported as bad once, during the drain.
			for _, block := range blocks {
				if bad := rawdb.ReadBadBlock(db, block.Hash()); bad == nil {
