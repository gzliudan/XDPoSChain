...
// TestInsertChainReportsTheIndexThatStoppedTheBatchOnAKnownBlock pins the one shape in which
// InsertChain hands ErrKnownBlock to a caller: the future tail of a batch runs into a block
// this node already executed, which stops the queueing. The sentinel is local and the index
// is that known block's, so a caller that can carry on hands the rest of the batch back in
// from there - lead by the known block, which is adopted or skipped - instead of reporting a
// batch that is being imported as a failure. Reaching the shape takes a block dated ahead of
// the local clock, so the downloader meets it and cancels the content processing, while the
// file importers replay historical blocks and do not.
func TestInsertChainReportsTheIndexThatStoppedTheBatchOnAKnownBlock(t *testing.T) {
	...
	if !errors.Is(err, ErrKnownBlock) {
		t.Fatalf("block %d: have %v want %v", n, err, ErrKnownBlock)
	}
	...
	if !IsLocalInsertError(err) {
		t.Fatal("a batch that ran into a block already on disk is a local condition")
	}
}
...
// TestInsertSideChainKeepsTheHeadWhenTheSegmentDoesNotBeatIt pins the third ending of a
// segment that stops on a known block: the blocks are stored, the head is already at or
// above them, so there is nothing to adopt. That is not an error - reporting one would make
// the downloader cancel and ask for the same range again, and the fork choice would not
// change on the retry - but the head must not move either.
