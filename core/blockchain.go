...
func newBlockChain(db ethdb.Database, cacheConfig *CacheConfig, engine consensus.Engine, vmConfig vm.Config, cfg blockchainOpenConfig) (*BlockChain, error) {
	...
	if compatErr != nil {
		...
		case MismatchRewindAndUpdate:
			...
			log.Warn("Applying chain config mismatch policy", "policy", compatPolicy, "rewind_to", compatErr.RewindTo, "update_config", true, "err", compatErr)
		...
		case MismatchUpdateConfigOnly:
			...
			log.Warn("Applying chain config mismatch policy", "policy", compatPolicy, "rewind", false, "update_config", true, "err", compatErr, "risk", "may cause state/consensus divergence; expert use only")
			rawdb.WriteChainConfig(db, genesisHash, chainConfig)
		case MismatchIgnoreMismatch:
			log.Warn("Applying chain config mismatch policy", "policy", compatPolicy, "rewind", false, "update_config", false, "err", compatErr, "risk", "may cause state/consensus divergence; expert use only")
		default:
