	...
	if len(blocks) > 0 {
		...
		for i := range blocks {
			...
			if _, err := bc.InsertChain(blocks[i : i+1]); err != nil {
				...
				// eviction is logged at Warn, including the ones the table blames the block for:
				// that report is not guaranteed. insertChain hands back unrecognised errors from
				// paths that never ask the table - the state read of its main loop - and the
				// default class still calls them the block's fault, so a Warn here is the only
				// trace such an eviction leaves. The ones the table does not blame the block for
				// - a refused reorg, an ancestor whose state is gone - have no report at all, and
				// dropping one of those is exactly what has to be visible.
				//
