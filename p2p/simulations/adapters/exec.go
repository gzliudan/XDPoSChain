...
func initLogging() {
	...
	if conf.Node.LogVerbosity >= 0 && conf.Node.LogVerbosity <= 5 {
		verbosity = log.FromLegacyLevel(conf.Node.LogVerbosity)
	}
	// Reinitialize the logger
	glogger = log.NewGlogHandler(log.NewTerminalHandler(writer, true))
