...
func TestDialSchedulerCheckDialAllowsWhenNotConnected(t *testing.T) {
	t.Parallel()
	...
	err := state.checkDial(newNode(id, ""))
	if err != nil {
		t.Fatalf("expected dial to be allowed for disconnected peer, got %v", err)
	}
}

func TestDialSchedulerCheckDialRejectsWhenConnected(t *testing.T) {
	t.Parallel()
	...
	if err != errAlreadyConnected {
		t.Fatalf("expected connected peer to be rejected, got %v", err)
	}
}
