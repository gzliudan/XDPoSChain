...
func TestUDP_pingPacketRejected(t *testing.T) {
	...
	enc, _, err := test.udp.encode(test.remotekey, &pingV4{From: testRemote, To: testLocalAnnounced, Version: 4, Expiration: futureExp})
	if err != nil {
		t.Fatalf("encode failed: %v", err)
	}
	enc[headSize] = p_pingV4
	sig, err := crypto.Sign(crypto.Keccak256(enc[headSize:]), test.remotekey)
	if err != nil {
		t.Fatalf("resign failed: %v", err)
	}
	copy(enc[macSize:], sig)
	hash := crypto.Keccak256(enc[macSize:])
	copy(enc, hash)
