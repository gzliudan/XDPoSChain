...
func TestUDPv4_packetErrors(t *testing.T) {
	...
	test.packetIn(errExpired, p_pingXdcV4, &pingV4{From: testRemote, To: testLocalAnnounced, Version: 4})
...
func TestUDP_pingPacketRejected(t *testing.T) {
	...
	enc, _, err := test.udp.encode(test.remotekey, p_pingV4, &pingV4{From: testRemote, To: testLocalAnnounced, Version: 4, Expiration: futureExp})
	if err != nil {
...
func TestUDPv4_pingMatch(t *testing.T) {
	...
	test.packetIn(nil, p_pingXdcV4, &pingV4{From: testRemote, To: testLocalAnnounced, Version: 4, Expiration: futureExp})
...
func TestUDPv4_successfulPing(t *testing.T) {
	...
	go test.packetIn(nil, p_pingXdcV4, &pingV4{From: testRemote, To: testLocalAnnounced, Version: 4, Expiration: futureExp})
...
func TestUDPv4_forwardCompatibility(t *testing.T) {
	...
	for _, test := range testPackets {
		...
		if _, isPing := test.wantPacket.(*pingV4); isPing {
			if err == nil || !strings.Contains(err.Error(), "unknown type") {
