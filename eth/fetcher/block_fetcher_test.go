...
// Tests that the consensus handler and the final broadcast are skipped when
// the import reported success without the block actually reaching the chain
// (e.g. the block or one of its ancestors was parked in the future queue):
// signing and voting on an unimported block would corrupt consensus state,
// while relaying it is safe and happens regardless.
