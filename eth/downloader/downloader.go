...
var (
	...
	// the intent: a batch is contiguous and splitBlocksForVerification only ever cuts it
	// between a block and its child, so a segment that failed leaves the next one without the
	// parent it has to link to - it could only fail as well. Synchronise re-selects a peer once
	// the cycle has ended, so the retry is the caller's, and the head staying where it is is
	// what makes the next attempt end the same way.
...
// segment that failed leaves the next one without the parent it has to link to - it could only
// fail as well.
//
