...
func TestFetcherWiresNoopHandlerWithoutConsensus(t *testing.T) {
	...
	for _, mode := range []downloader.SyncMode{downloader.FastSync, downloader.FullSync} {
		...
		defer pm.Stop()
		// Read the wired handler through the fetcher's accessor instead of a
		// ProtocolManager field: the accessor is the single source of truth for
		// what NewBlockFetcher was handed (which also normalizes nil to no-op).
		wired := pm.blockFetcher.ProposedBlockHandler()
		// A nil check here would be unreachable: NewBlockFetcher normalizes a nil
		// handler to a no-op, so the accessor can never return nil.
...
// keeps it, so this accessor returns nil and callers nil-check (as the import
...
func TestDownloaderKeepsNilHandlerWithoutConsensus(t *testing.T) {
	for _, mode := range []downloader.SyncMode{downloader.FastSync, downloader.FullSync} {
		pm, _ := newTestProtocolManagerMust(t, mode, 0, nil, nil)
		defer pm.Stop()
		if wired := pm.downloader.ProposedBlockHandler(); wired != nil {
...
// block through; a block whose state was never executed is skipped with a
// Warn and counted, independent of the snapSync flag. A nil header or nil
// number fails the SkipNilHeader guard (Error, uncounted) without panicking.
...
// skipped-proposed-block root (engine gates, downloader pre-filter, this
// gate) so one alert regex aggregates them; a silent rename would orphan
// external alerts.
...
func TestFetcherWiresGatedHandlerWithXDPoS(t *testing.T) {
	...
	// NewBlockFetcher normalizes a nil handler to a no-op, so ProposedBlockHandler
	// can never return nil and a nil check here would be unreachable. An
...
func TestDownloaderWiresUngatedHandlerWithXDPoS(t *testing.T) {
	...
	// wiring regression.
	wired := pm.downloader.ProposedBlockHandler()
	if wired == nil {
